...
이는 negative entropy를 나타내게 되고(분모), expectation of conditional entropy - entropy 를 분자에 넣어 비를 구한다.

cross entropy =  $- \sum _j \pi_{+j} log \pi_{i+}$ ;; 
conditional entropy = $- \sum _j \pi_{j \mid i} log \pi_{j \mid i}  $ 


$$
(expectation \ \ of \ \ conditional \ \ entropy - entropy ) / (entropy) =  \frac{ - \sum_i \pi_{i+} \sum _j \pi_{j|i} log \pi_{j|i} - (-\sum _j \pi_{+j}log \pi_{+j})}{- \sum _j \pi_{+j}log \pi_{+j}} =  \frac{\sum _j \pi_{+j}log \pi_{+j} - \sum_i \pi_{i+} \sum _j \pi_{j|i} log \pi_{j|i}}{\sum _j \pi_{+j}log \pi_{+j}} = - \frac {\sum_i\sum_j\pi_{i,j}log(\pi_{i,j}/\pi_{i+}\pi_{+j})}{\sum _j \pi_{+j}log \pi_{+j}}
$$
