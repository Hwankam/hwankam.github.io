...
sensitivity : P( Diagnosis of Test is Positive | person has disease )

specificity : P( Diagnosis of Test is Negative | person doesn't have disease )
...
$P(Y_{ij}= n_{ij}) = \Pi_i [\frac {n_i !} {\Pi_j (n_{ij}!)} \  \Pi_j \pi_{j|i}^{n_{ij}}]$
