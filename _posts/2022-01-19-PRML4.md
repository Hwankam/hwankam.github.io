...
Input space를 K개의 Class로 나누는 것!

이때 나눠지는 영역은 decision region이며 boundary는 decision boundary 혹은 decision surface라 한다. 이번 chapter에서 중요한 것은 linear model이다. 즉 분류를 위한 decision surface가 linear function으로서 D-dimensional input space를 (D-1)-dimensional hyperplane으로 나누는 것을 의미한다. 
...
$\&$ 
decision stage (use dist to make decision )

    then, how to determine $p(C_k|\mathcal{x})?$
...
classification에서는 linear model의 결과값이 discrete class label과 관련 있어야 하기 때문에 비선형함수인 activation function을 사용한다. (**generalized** linear model )

이 장의 뒷부분에는 input space $\mathcal{X}$를 
...
Discriminant 라는 것은 말 그대로 구분식! 즉, input $\mathcal{X}$를 어떤 Class에 넣을 것인지 결정해준다!

여기서는 특별히 linear discriminants 에 한정해서 설명하도록 하겠다. 
...
Simplest representation of a linear discriminant
...
$\boldsymbol{w}$ 는 weight vector이며 decision boundary에 직교하는 벡터이므로 decision boundary의 방향을 결정해준다. 
$\ w_0$ 는 bias이며 동시에 negative bias는 threshold. 

negative bias 가 threshold 인 이유 

* $y(\boldsymbol{x}) >0 $ 이면 Class 1 으로 판정하기 때문 

* $\boldsymbol{x}$가 decision boundary에 위치할 때, 원점에서 decision boundary 까지 거리를 normalizing 하면 
...
$\gamma$의 값이 중요하므로, 위와 같이 구할 수 있다. 



또한 bias를 dummy input을 활용해 $\boldsymbol{\tilde w} = (w_0, \boldsymbol{w})$ 라 할 때, 
$y(\boldsymbol{x}) = \boldsymbol{\tilde w}^T \boldsymbol{\tilde x}$ 로 간단히 표현할 수 있으며 hyperplane( boundary)는 원점을 통과하는 D - dimensional hyperplane 이다. 
...
binary class 와 유사하게 class k 와 class j 사이의 boundary는 $y_k(\boldsymbol{x}) = y_j(\boldsymbol{x})$ 를 만족하는 식인 
